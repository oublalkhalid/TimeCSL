...
class TimeseriesDataset(torch.utils.data.Dataset):
    ...
    def load_data(self):
        ...
        print("self.target_in_dataset", self.target_in_dataset, "df.columns", df.columns)
    ...
    def scaling(self, infernece : bool = False):
        if infernece:
            ...
            if self.normalization_type == "standarization":
                ...
                self.X_mean =  np.array([params_appliance['all_device']['mean'] for m in self.features])
                self.X_std =  np.array([params_appliance['all_device']['std'] for m in self.features])
                ...
                # self.X = self.scaler_agg.transform(self.X)
                # self.y = self.X
                self.X_min =  np.array([params_appliance['all_device']['min'] for m in self.features])
                self.X_max =  np.array([params_appliance['all_device']['max'] for m in self.features])
    ...
    def __getitem__(self, index):
        ...
        # length_seq_out
        x = self.padding_seqs(self.X[start_index: end_index], window_size=self.window_size)
        y = self.padding_seqs(self.y[start_index: end_index_target], window_size=self.length_seq_out)
        ...
        y_unorm = self.padding_seqs(self.y_unorm[start_index: end_index_target], window_size=self.length_seq_out)
        embedings = self.padding_seqs(self.y[start_index-1: end_index-1], window_size=self.window_size)
        index = self.padding_seqs(self.index[start_index: end_index_target], window_size=self.window_size)
        ...
        if len(y.shape)>1:
            ...
            index, x, y, embedings = torch.Tensor(index), torch.Tensor(x.transpose(1,0)), torch.Tensor(y.transpose(1,0)), torch.Tensor(embedings.transpose(1,0))
            y_unorm = torch.Tensor(y_unorm.transpose(1,0))
        else:
            y = y.reshape(y.shape[0],1)
            y_unorm = y_unorm.reshape(y.shape[0],1)
            y_unorm = torch.Tensor(y_unorm)
            x, y, embedings = torch.Tensor(x.transpose(1,0)), torch.Tensor(y), torch.Tensor(embedings)
    ...
    def apply_window_in(self):
            self.X, self.y = np.concatenate((np.zeros((self.window_size,self.X.shape[1])),self.X)), np.concatenate((np.zeros((self.window_size,self.y.shape[1])),self.y))
            self.index = np.arange(0,len(self.X))

    def apply_window_end(self):
            self.X, self.y = np.concatenate((self.X, np.zeros((self.window_size,self.X.shape[1])))), np.concatenate((self.y, np.zeros((self.window_size,self.y.shape[1]))))
            self.index = np.arange(0,len(self.X))
...
class ParallelTraining(Datasource_BANKNILM):
    def __init__(self,
        ...
        for indice in t:
            ...
            dataset = TimeseriesDataset(h5_path=self.hdf5_dataset,
                                        house_index = indice, 
    ...
    def inverse_transform(self,agg_data,label_data = None):
        if label_data is None:
            agg_data_inverse = self.combination[0].scaler_agg.inverse_transform(agg_data)
    ...
    def prams(self):
        ...
        if self.testing_phase:
            print('######## TEST PARMAS ###############')
            self.maxs_agg,self.means_agg,self.sigma_agg,self.maxs_disag,self.means_disag,self.sigma_disag = get_params(path_dataset=self.backend_dataset, 
                                                                                            targets=self.targets, infos_agg="infos_agg_test",
                                                                                            infos_disag="infos_disag_test")                                                                                      
        else:
            print('######## TRAIN PARMAS ###############')
            self.maxs_agg,self.means_agg,self.sigma_agg,self.maxs_disag,self.means_disag,self.sigma_disag = get_params(path_dataset=self.backend_dataset, 
                                                                                            targets=self.targets, infos_agg="infos_agg_train",
                                                                                            infos_disag="infos_disag_train")
        self.DATASET.close()

        return (self.resolution,
                self.maxs_agg,self.means_agg,self.sigma_agg,
                self.maxs_disag,self.means_disag,self.sigma_disag)
    
    def train_val_dataloader(self):
        train_dataset, val_dataset = random_split(self.dataset, [self.train_size, self.val_size],
                                                          generator=torch.Generator().manual_seed(1234))
        train_loader = DataLoader(train_dataset, batch_size=self.batch_size,
                                        shuffle=True, num_workers=0,pin_memory=False, drop_last=False)#0, pin_memory=False)
...
class TimeseriesDataset_csv(torch.utils.data.Dataset,Processing_data):
    ...
    def __getitem__(self, index):
        ...
        x = self.padding_seqs(self.X[start_index: end_index], window_size=self.window_size)
        ind = self.padding_seqs(self.index[start_index: end_index], window_size=self.window_size)
    ...
    def create_dataset(self):
        ...
        if len(self.appliance_to_track) > 1:
            X_mean = np.array([params_appliance['all_device']['mean'] for k in self.features])
            X_std = np.array([params_appliance['all_device']['std'] for k in self.features])
        else:
            X_mean = np.array([params_appliance[k]['mean'] for k in self.appliance_to_track for m in self.features])
            X_std = np.array([params_appliance[k]['std'] for k in self.appliance_to_track for m in self.features])
